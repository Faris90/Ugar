...
function Blob(server, nodeId, x, y, mass, parent) {
  ...
  var color = {
	...
	//this.hue = {"r":color.r,"g":color.g,"b":color.b};
	var colorRGB = [0xFF, 0x07, ((Math.random() * (256 - 7)) >> 0) + 7]; //Vanilla color scheme
...
PlayerBlob.prototype.canCombine = function() {
	var required = 0.15 * this.mass + this.server.config.baseTime * 1000;
...
Player.prototype.setSkin = function(n) {
	this.skin = n;
};
Player.prototype.getSkin = function() {
	return this.skin == "" ? "Skinless" : this.skin;
};
...
function Server() {
	...
	this.config = {
		...
		playerStartMass: 10,
		playerMinMassForSplit: 32,
		playerMinMassForEject: 32,
		playerMaxMass: 20000,
		playerMaxSplit: 16,
		rangeWidth: 5000,
		rangeHeight: 5000,
...
Server.prototype.createPlayerBlob = function(x, y, mass, angle, parentBlob, parent) {
	...
	playerBlob.skin = parent.getSkin();
